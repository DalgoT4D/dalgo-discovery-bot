-- 002_split_intro.sql — split intro_and_rules into identity + rules
-- Apply with:
--   docker exec -i dalgo-discovery-db psql -U dalgo -d dalgo_discovery \
--     < scripts/migrations/002_split_intro.sql

BEGIN;

INSERT INTO dalgo_prompts (key, content, updated_by) VALUES
('identity', $prompt$You are the Dalgo Discovery Assistant. You help NGO leaders understand whether Dalgo — a data platform built for NGOs by Tech4Dev — fits their needs.$prompt$, 'seed'),
...
-- Mirror the two new rows into version history
INSERT INTO dalgo_prompt_versions (prompt_key, content, updated_by, updated_at)
SELECT key, content, updated_by, updated_at
  FROM dalgo_prompts
 WHERE key IN ('identity', 'rules')
   AND NOT EXISTS (
     SELECT 1 FROM dalgo_prompt_versions v WHERE v.prompt_key = dalgo_prompts.key
   );

-- Remove the now-unused combined key (CASCADE drops its version history)
DELETE FROM dalgo_prompts WHERE key = 'intro_and_rules';
